#!/usr/bin/env python3
"""
Filter dental_clinics_bay_of_plenty.csv (raw Places API results) down to
genuine new dental clinics and merge into dental_clinics_all.csv, following
the same approach used for the Wellington and Waikato merges.
"""
import csv
import re
...
suburb_map = {}
for r in existing:
    if r["region"] == "Bay of Plenty":
        suburb_map[r["suburb_town"].strip().lower()] = (r["city"], r["region"])

EXCLUDE_NAME = re.compile(
    r"\b(pharmacy|chemist|unichem|vets?|veterinary|animates|hospital|"
    r"medical centre|medical practice|medical care|medical limited|medical store|tui medical|after.?hours|"
    ...
    r"radiology|community dental|diagnostic centre|te whatu ora|"
    r"birthing centre|elder abuse|beauty|urban retreat|mall\b|college\b)\b",
    re.IGNORECASE,
)
EXCLUDE_EXACT: set[str] = {
    "tauranga hospital",
    "dental implant centre",  # generic name dup of address, low confidence
    "rotorua intermediate dental clinic",  # school dental clinic
    "the cosmetic clinic rotorua",  # cosmetic injectables clinic, not dental
    "the cosmetic clinic bayfair (tauranga)",  # cosmetic injectables clinic, not dental
    "lakes primecare",  # GP/urgent-care practice, not dental
    "dental monitoring unit",  # vague internal unit, no standalone business presence
    "duncan dental 11th ave (moved to 186 fraser street)",  # stale listing, superseded by Duncan Dental Fraser Street
    "te ao marama aotearoa maori dental association",  # professional association, not a clinic
}
...
with open("dental_clinics_bay_of_plenty.csv", encoding="utf-8", newline="") as f:
