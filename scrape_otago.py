#!/usr/bin/env python3
"""
Scrape dental clinics in the Otago region using the Google Places API (New).
Outputs results to dental_clinics_otago_fresh.csv

Covers: Dunedin (searched by suburb to maximise coverage), Queenstown Lakes,
Central Otago, Waitaki, and Clutha districts.

Requires:
    GOOGLE_PLACES_API_KEY environment variable to be set.

Usage:
    python3 scrape_otago.py
"""
...
OUTPUT_FILE = "dental_clinics_otago_fresh.csv"
REGION = "Otago"

TOWNS = [
    # Dunedin City (searched by suburb to maximise coverage)
    "Dunedin Central",
    "Dunedin North",
    "Mosgiel",
    "Green Island",
...
# Address fragments used to keep only Otago-region results
OTAGO_REGION_MARKERS = [
    "Dunedin", "Mosgiel", "Green Island", "Roslyn", "Wakari", "Musselburgh",
    "North East Valley", "Kaikorai", "Mornington", "South Dunedin",
    "Port Chalmers", "St Clair", "St Kilda", "Waverley", "Maori Hill",
    "Andersons Bay", "Concord", "Queenstown", "Frankton", "Arrowtown",
    "Wanaka", "Wānaka", "Alexandra", "Cromwell", "Roxburgh", "Ranfurly",
    "Oamaru", "Palmerston", "Balclutha", "Milton", "Tapanui", "Lawrence",
    "Otago",
]
...
def in_region(address: str) -> bool:
    return any(marker in address for marker in OTAGO_REGION_MARKERS) and address.strip().endswith("New Zealand")
