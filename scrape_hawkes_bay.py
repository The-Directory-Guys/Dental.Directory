#!/usr/bin/env python3
"""
Scrape dental clinics in the Hawke's Bay region using the Google Places API (New).
Outputs results to dental_clinics_hawkes_bay.csv

Requires:
    GOOGLE_PLACES_API_KEY environment variable to be set.

Usage:
    python3 scrape_hawkes_bay.py
"""
...
OUTPUT_FILE = "dental_clinics_hawkes_bay.csv"
REGION = "Hawke's Bay"
...
def main():
    ...
    for town in HAWKES_BAY_TOWNS:
        ...
        for place in places:
            ...
            if not any(t in address for t in HAWKES_BAY_TOWNS) and "Hawke" not in address:
                continue
