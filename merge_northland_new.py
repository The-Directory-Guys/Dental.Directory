#!/usr/bin/env python3
"""
Filter dental_clinics_northland.csv (raw Places API results) down to
genuine new dental clinics and merge into dental_clinics_all.csv, following
the same approach used for the Wellington/Waikato/BoP/Otago/MW merges.
"""
import csv
import re
...
EXCLUDE_NAME = re.compile(
    ...
    r"new world|pak.?nsave|freshchoice|countdown|woolworths|the warehouse|mitre 10|mcdonald|"
    ...
    r"family care centre|hauora|animal bedding|fono\b)\b",
    re.IGNORECASE,
)
EXCLUDE_EXACT: set[str] = {
    "dentist",  # no business name, junk listing
    "whangarei",  # bare place-name listing, not a real business
    "rawene clinic",  # generic GP-style clinic name, not confirmed dental
    "take ngakau",  # not a dental business
    "yourvet",  # vet clinic (no word-boundary match for "yourvet")
    "oral health clinic",  # Northland DHB free community oral health service
    "te hau ora o ngāpuhi",  # Maori health provider, not dental
}
...
with open("dental_clinics_northland.csv", encoding="utf-8", newline="") as f:
