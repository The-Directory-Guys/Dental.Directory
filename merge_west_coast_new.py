#!/usr/bin/env python3
"""
Filter dental_clinics_west_coast_fresh.csv (raw Places API results) down to
...
EXCLUDE_NAME = re.compile(
    ...
    r"the doctors|church\b|community health centre|health\b|retreat\b|"
    r"optometrist|health center)\b",
    re.IGNORECASE,
)
EXCLUDE_EXACT: set[str] = {
    "dentist",  # no business name, junk listing
    "franz josef clinic",  # vague generic clinic name, not confirmed dental
}
...
with open("dental_clinics_west_coast_fresh.csv", encoding="utf-8", newline="") as f:
