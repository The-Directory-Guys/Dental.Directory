#!/usr/bin/env python3
"""
Scrape dental clinics in the Marlborough region using the Google Places API (New).
Outputs results to dental_clinics_marlborough_fresh.csv

Covers: Blenheim (searched by suburb to maximise coverage), Picton,
Renwick, Havelock, and other Marlborough towns.

Requires:
    GOOGLE_PLACES_API_KEY environment variable to be set.

Usage:
    python3 scrape_marlborough.py
"""
...
OUTPUT_FILE = "dental_clinics_marlborough_fresh.csv"
REGION = "Marlborough"

TOWNS = [
    # Blenheim (searched by suburb to maximise coverage)
    "Blenheim Central",
    "Burleigh Blenheim",
    "Springlands",
    "Redwoodtown",
...
# Broader markers for the in-region address check
REGION_MARKERS = TOWNS + ["Blenheim", "Marlborough"]
...
def in_region(address: str) -> bool:
    return any(marker in address for marker in REGION_MARKERS) and address.strip().endswith("New Zealand")
