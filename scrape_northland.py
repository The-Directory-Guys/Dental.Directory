#!/usr/bin/env python3
"""
Scrape dental clinics in the Northland region using the Google Places API (New).
Outputs results to dental_clinics_northland.csv

Requires:
    GOOGLE_PLACES_API_KEY environment variable to be set.

Usage:
    python3 scrape_northland.py
"""
...
OUTPUT_FILE = "dental_clinics_northland.csv"
REGION = "Northland"
...
# Broader markers for the in-region address check -- Google often returns
# addresses as just "Whangārei 0110" without "Central"/"Regent"/etc, so the
# per-suburb search-query strings above aren't enough on their own.
REGION_MARKERS = NORTHLAND_TOWNS + [
    "Whangārei", "Whangarei", "Kamo", "Northland",
]
...
def main():
    ...
    for town in NORTHLAND_TOWNS:
        ...
        for place in places:
            ...
            if not any(t in address for t in REGION_MARKERS) and REGION not in address:
                continue
