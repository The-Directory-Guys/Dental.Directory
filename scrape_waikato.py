#!/usr/bin/env python3
"""
Scrape dental clinics in the Waikato region using the Google Places API (New).
Outputs results to dental_clinics_waikato.csv

Covers: Hamilton, Thames-Coromandel, Hauraki, Waikato district, Waipā,
Ōtorohanga, South Waikato, Waitomo, and Taupō districts.

Requires:
    GOOGLE_PLACES_API_KEY environment variable to be set.

Usage:
    python3 scrape_waikato.py
"""
...
OUTPUT_FILE = "dental_clinics_waikato.csv"
REGION = "Waikato"

TOWNS = [
    # Hamilton City (searched by suburb to maximise coverage)
    "Hamilton Central",
    "Frankton Hamilton",
    "Hillcrest Hamilton",
    "Chartwell Hamilton",
...
# Address fragments used to keep only Waikato-region results
WAIKATO_REGION_MARKERS = [
    "Hamilton", "Waikato", "Ngaruawahia", "Huntly", "Raglan",
    "Te Awamutu", "Cambridge", "Matamata", "Morrinsville", "Te Aroha",
    "Thames", "Whitianga", "Tairua", "Whangamata", "Waihi", "Paeroa",
    "Ngatea", "Otorohanga", "Te Kuiti", "Tokoroa", "Putaruru", "Tirau",
    "Taupo", "Taupō", "Turangi", "Tūrangi", "Mangakino", "Frankton",
    "Coromandel", "Pirongia",
]
...
def in_region(address: str) -> bool:
    return any(marker in address for marker in WAIKATO_REGION_MARKERS)
