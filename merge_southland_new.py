#!/usr/bin/env python3
"""
Filter dental_clinics_southland_fresh.csv (raw Places API results) down to
...
EXCLUDE_NAME = re.compile(
    ...
    r"hungry pet|motel\b|aged residential care|"
    r"kindergarten|care village|takeaways|prescriptions and gifts|"
    r"school\b)\b",
    re.IGNORECASE,
)
EXCLUDE_EXACT: set[str] = {
    "dentist",  # no business name, junk listing
    "vetsouth otautau",  # vet clinic (no word-boundary match for "vetsouth")
    "vetsouth lumsden",  # vet clinic (no word-boundary match for "vetsouth")
}

DENTAL_OVERRIDE = re.compile(r"\b(lumino|dentist|dentists)\b", re.IGNORECASE)

MANUAL_SUBURB_MAP = {
    "te anau": ("Te Anau", "Southland"),
...
with open("dental_clinics_southland_fresh.csv", encoding="utf-8", newline="") as f:
    raw = list(csv.DictReader(f))

# "Wyndham" is ambiguous (a street in Auckland CBD as well as a Southland
# town) -- drop anything whose address is actually in Auckland.
raw = [r for r in raw if "Auckland" not in r["address"]]
...
print(f"Raw (after Auckland leak removal): {len(raw)}  After noise filter: {len(kept)}")
