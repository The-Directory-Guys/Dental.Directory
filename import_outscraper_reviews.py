...
CSV_PATH = os.path.join(os.path.dirname(__file__), 'outscraper_batch3.csv')
PROGRESS_PATH = os.path.join(os.path.dirname(__file__), 'outscraper_progress_batch3.json')
RAW_PATH = os.path.join(os.path.dirname(__file__), 'outscraper_raw_responses_batch3.jsonl')

BATCH_SIZE = 10  # URLs per Outscraper request
...
def fetch_reviews(urls):
    """Call Outscraper Google Maps Reviews API for a batch of URLs."""
    params = {
        'query': urls,
        'reviewsLimit': 20,
        'language': 'en',
    ...
    # API returns a list of lists — one inner list per URL; take the first result from each
    places = []
    for item in raw:
...
def get_existing_reviews(clinic_id):
    """Fetch existing (author, date_text) pairs to deduplicate."""
    resp = requests.get(
