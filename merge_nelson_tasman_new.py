#!/usr/bin/env python3
"""
Filter dental_clinics_nelson_tasman_fresh.csv (raw Places API results) down
to genuine new dental clinics and merge into dental_clinics_all.csv,
following the same approach used for the previous region merges.
"""
import csv
import re
...
EXCLUDE_NAME = re.compile(
    ...
    r"hungry pet|motel\b|aged residential care|school dental clinic)\b",
    re.IGNORECASE,
)
EXCLUDE_EXACT: set[str] = {
    "dentist",  # no business name, junk listing
    "mapua school dental clinic",  # school dental clinic
    "greenwood health",  # general health practice, not dental
}

DENTAL_OVERRIDE = re.compile(r"\b(lumino|dentist|dentists)\b", re.IGNORECASE)

MANUAL_SUBURB_MAP = {
    "brightwater": ("Brightwater", "Nelson & Tasman"),
}
...
with open("dental_clinics_nelson_tasman_fresh.csv", encoding="utf-8", newline="") as f:
    raw = list(csv.DictReader(f))

# "Richmond" is ambiguous (a Christchurch suburb as well as a Nelson/Tasman
# town) -- drop anything whose address is actually in Christchurch.
raw = [r for r in raw if "Christchurch" not in r["address"]]
...
print(f"Raw (after Christchurch leak removal): {len(raw)}  After noise filter: {len(kept)}")
