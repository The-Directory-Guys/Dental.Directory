#!/usr/bin/env python3
"""
Filter dental_clinics_marlborough_fresh.csv (raw Places API results) down
...
EXCLUDE_NAME = re.compile(
    ...
    r"the doctors|church\b|community health centre|health\b|cottage\b)\b",
    re.IGNORECASE,
)
EXCLUDE_EXACT: set[str] = {
    "dentist",  # no business name, junk listing
    "gilchrist dental",  # stale duplicate listing of existing "Beattie Rose and Gilchrist" (same address, no contact info)
}

DENTAL_OVERRIDE = re.compile(r"\b(lumino|dentist|dentists)\b", re.IGNORECASE)

MANUAL_SUBURB_MAP = {
    "picton": ("Picton", "Marlborough"),
}
...
with open("dental_clinics_marlborough_fresh.csv", encoding="utf-8", newline="") as f:
